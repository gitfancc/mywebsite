...
JustGage = function(config) { 
  ...
  this.config = 
  {
    ...
    titleFontColor : (config.titleFontColor) ? config.titleFontColor : "#666666",
    ...
    // value alias
   /// aliasValue : (config.aliasValue) ? aliasValue : 0,
...
// refresh gauge level
JustGage.prototype.refresh = function(val, alias) {
  // overflow values
  originalVal = val;
  ...
  if(this.config.id == "sleep")
	  this.canvas.getById(this.config.id+"-txtvalue").attr({"text":alias});
  else
...
JustGage.prototype.refreshEx = function(val, alias, max) {
	this.config.max = max;
	this.canvas.getById(this.config.id+"-txtmax").attr({"text":max});
...
JustGage.prototype.refreshTips = function(val) {
	  this.canvas.getById(this.config.id+"-txttitle").attr({"text":val});
};
